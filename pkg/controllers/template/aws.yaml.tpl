...
spec:
  kubeadmConfigSpec:
    clusterConfiguration:
      apiServer:
        extraArgs:
          ...
          service-account-issuer: https://{{ .S3BucketName }}.s3.amazonaws.com
    ...
    {{- if .S3BucketName }}
    preKubeadmCommands:
    - aws s3 cp s3://{{ .S3BucketName }}/sa-signer-pkcs8.pub /etc/kubernetes/pki/sa-signer-pkcs8.pub
    - aws s3 cp s3://{{ .S3BucketName }}/sa-signer.key /etc/kubernetes/pki/sa-signer.key && chmod 0600 /etc/kubernetes/pki/sa-signer.key
    {{- end }}
  machineTemplate:
    infrastructureRef:
      apiVersion: infrastructure.cluster.x-k8s.io/v1beta2
