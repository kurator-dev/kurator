...
func (a *ApplicationManager) SyncPolicyResource(ctx context.Context, app *applicationapi.Application, fleet *fleetapi.Fleet, syncPolicy *applicationapi.ApplicationSyncPolicy) error {
	...
	for _, fleetCluster := range fleetClusterList {
		// fetch kubeconfig for each cluster.
		kubeConfig := a.generateKubeConfig(fleetCluster)

		// handle gitRepo + kustomization
		if sourceKind == GitRepoKind {
			kustomization := syncPolicy.Kustomization
			kustomizationName := generateKustomizationName(app, syncPolicy, fleetCluster.GetObject().GetObjectKind().GroupVersionKind().Kind, fleetCluster.GetObject().GetName())
		...
		if sourceKind == HelmRepoKind {
			...
			helmReleaseName := generateHelmReleaseName(app, syncPolicy, fleetCluster.GetObject().GetObjectKind().GroupVersionKind().Kind, fleetCluster.GetObject().GetName())
